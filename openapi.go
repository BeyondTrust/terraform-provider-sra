...
func ParseOpenAPI() {
	...
	err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		log.Printf("📄 Processing doc path %s\n", path)
		if err != nil {
			return err
		}

		if key, processFile := typeMap[path]; processFile {
			log.Printf("📄 Processing doc file %s (%s)\n", path, key)
			newLines := []string{}
...
func extractVal(val []string, item map[string]interface{}) (*string, error) {
	...
	for i, v := range val {
		if i == len(val)-1 {
			...
			if !ok {
				log.Printf("🛑 failed to convert string %+v [%v]", currentLevel, v)
				return nil, fmt.Errorf("failed to convert string")
			}
		} else {
			currentLevel, ok = currentLevel[v].(map[string]interface{})
			if !ok {
				log.Printf("🛑 failed to convert map %+v [%v]", currentLevel, v)
